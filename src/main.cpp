...
int main(int, char**) {
  if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_TIMER) != 0) {
    std::cerr << "Unable to initialize SDL_Init " << SDL_GetError()
              << std::endl;
    return 1;
  }
  ...
  if (window == nullptr) {
    std::cerr << "Unable to initialize SDL_Window, exiting." << SDL_GetError()
              << std::endl;
    return 1;
  }
  ...
  if (context == nullptr) {
    std::cerr << "Unable to initialize OpenGL context, exiting."
